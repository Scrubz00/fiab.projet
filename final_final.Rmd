...
---
![](D:/Master_1_mes_cours_et_project/Simulation_R/simulation/logo.png)

\noindent \hrulefill
 \vspace*{7mm}  

\begin{center}\begin{Huge}\textbf{Projet de fiabilité}\end{Huge} \end{center}
\vspace*{5mm}
\begin{center}\begin{Large}\textbf{Étude de systèmes complexes}\end{Large} \end{center}
\vspace*{5mm}

   \begin{center}\begin{Large}MASTER 2 : Mathématique appliquée, parcours modélisation statistique\end{Large}\end{center}
 \vspace*{5mm}  
...
\textbf{Younes Chahma et Camille Giampiccolo}
\end{Large}\end{center}
\begin{Large}
\vspace*{5mm}  

\begin{center}Encadré par \end{center} 
\end{Large}
\vspace*{5mm} 

\begin{center}\begin{Large}\textbf{Professeur Célestin C. Kokonendji }\end{Large} \end{center}
\vspace*{5mm}
\begin{center}24 novembre 2020\end{center}

\noindent \hrulefill 

\begin{center}
\includegraphics{C:/Users/Camille/Desktop/fiab.projet/fiab}
\end{center}
...
La deuxième simulation est représentée par la courbe en pointillés.
...
Pour la première simulation, on observe que à $t=17$ le système tombe en panne.  
...
On considére que $(T_{1_i})_{i=1}^n$ est une variable aléatoire indépendante et identiquement distribuée. 
...
g <- ggplot() +
  ...
  geom_point()+
  labs(title="Fonction renvoyant la différence",
       x = "beta",
       y = "inegalite_booleen")
g
```
...
Dans cette seconde partie, nous étudierons un autre système complexe, composé de 10 composants. Le but de cette partie sera de faire de nombreuse simulation et voir ce qui se passe dans le cas où un composant est réparable. Nous étudierons aussi l'impacte financier des pannes pour une entreprise.
Dans cette partie, nous éviterons de mettre les morceaux de code car nous en avons déja beaucoup mis dans la partie précédente et dans cette partie, les code sont plus ou moins similaires.
...
$\phi(x) = [x_5(1-(1-x_1)(1-x_2))(1-(1-x_3)(1-x_4))+(1-x_5)(1-(1-x_1x_3)(1-x_2x_4))]*$   \hspace*{1.6cm}$x_6[1-(1-x_7x_{10})(1-x_8x_9)]$
...
nb_intervention <- function(a, n, lambda, beta,inter){
  ...
  for(i in 1:n){
    ...
    if((fonction_interval(n,inter,i)==1) && (phi(x[i,]) == 1)){
...
Ce projet est intéressant dans le sens où on a mis en pratique des connaissances que nous avions aquis théoriquement cette année.   
On a pu visualiser comment évoluent les risques, les durées de vie d'un système en fonction de différents paramètres de la modélisation. 
